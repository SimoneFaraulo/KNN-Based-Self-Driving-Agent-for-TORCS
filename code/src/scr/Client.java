...
public class Client {
	 ...
	 *             connessione (il valore predefinito è 3001).
	 *             - host:INDIRIZZO viene utilizzato per specificare l'indirizzo
	 *             dell'host dove il server è in esecuzione (il valore predefinito è
	 *             localhost).
	 *             - id:ClientID viene utilizzato per specificare l'ID del client
	 *             inviato al server (il valore predefinito è championship2009).
	 *             - verbose:on viene utilizzato per attivare la modalità verbose
	 *             (il valore predefinito è spento).
	 *             - maxEpisodes:N viene utilizzato per impostare il numero di
	 *             episodi (il valore predefinito è 1).
	 *             - maxSteps:N viene utilizzato per impostare il numero massimo di
	 *             passaggi per ogni episodio (il valore predefinito è 0, che
	 *             significa numero illimitato di passaggi).
	 *             - stage:N viene utilizzato per impostare lo stadio corrente: 0 è
	 *             WARMUP, 1 è QUALIFYING, 2 è RACE, altri valori significano
	 *             UNKNOWN (il valore predefinito è UNKNOWN).
	 *             - trackName:nome viene utilizzato per impostare il nome della
	 *             pista attuale.
	 */
	...
	public static void main(String[] args) {
		...
		String inMsg;
		KNearestNeighbor knn = new KNearestNeighbor("dataset.csv");
		...
		//Legge dal terminale un carattere sulla base del quale verrà eseguita la guida in una delle due modalità previste
		do {
			System.out.print("Digita \"a\" per testare l'auto in guida autonoma o \"m\" per pilotare l'auto manualmente: ");	
			mod = sc.nextLine().toLowerCase();
		} while (!mod.equals("a") && !mod.equals("m"));
		...
		do {
			...
			while (true) {
				...
				if (inMsg != null) {
					...
					//Esegue la logica per eseguire la guida in modalità autonoma
					if (mod.equals("a")) {
						MessageBasedSensorModel msg = new MessageBasedSensorModel(inMsg);

						Sample s = new Sample(msg.getAngleToTrackAxis(), msg.getSpeed(), msg.getLateralSpeed(),
								msg.getTrackEdgeSensors(), msg.getTrackPosition());

						Classe predicted = knn.classify(s, 5);

						Action action = new Action();

						action.accelerate = predicted.getAccelerate();
						action.brake = predicted.getBrake();
						action.clutch = 0;
						action.gear = predicted.getGear();
						action.steering = predicted.getSteering();

						mySocket.send(action.toString());
					}

					//Esegue la logica per eseguire la guida in modalità manuale
					if (mod.equals("m")) {
						Action action = new Action();
